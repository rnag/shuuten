...
def lambda_console_link(region: str, function_name: str) -> str:
    return (
        f'{AWS_CONSOLE}/lambda/home?region={quote(region)}'
        f'#/functions/{quote(function_name)}'
    )


def cloudwatch_log_stream_link(
    region: str, log_group: str, log_stream: str | None = None
) -> str:
    ...
    if log_stream:
        ...
        return (
            f'{AWS_CONSOLE}/cloudwatch/home?region={quote(region)}'
